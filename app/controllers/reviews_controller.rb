class ReviewsController < ApplicationController
  ...
  def current_user_must_be_review_account
    review = Review.find(params[:id])

    unless current_user == review.account
  ...
  def index
    ...
    @reviews = @q.result(:distinct => true).includes(:restaurants, :photos, :responses, :account, :owners).page(params[:page]).per(10)
  ...
  def create
    ...
    @review.restaurants_id = params[:restaurants_id]
    @review.account_id = params[:account_id]
